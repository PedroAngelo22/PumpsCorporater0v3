...
class PDFReport(FPDF):
    ...
    def add_key_value_table(self, data_dict):
        ...
        for key, value in data_dict.items():
            ...
            self.set_font('', 'B')
            self.cell(70, 8, f'{key}:', border=1) # Largura ajustada
            self.set_font('', '')
    ...
    def add_network_summary_table(self, network_data):
        ...
        draw_rows('Trechos em Série (Sucção)', network_data.get('antes', []))
        ...
        draw_rows('Trechos em Série (Recalque)', network_data.get('depois', []))
        self.ln(5)
...
def generate_report(project_name, scenario_name, params_data, results_data, metrics_data, 
                    network_data, diagram_image_bytes, chart_figure_bytes, npsh_data):
    ...
    pdf.add_section_title('Análise de Cavitação (NPSH)')
