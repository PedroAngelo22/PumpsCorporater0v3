...
# NOVO: Função para calcular o NPSH disponível
def calcular_npsh_disponivel(trechos_succao, vazao_op_m3h, params_npsh, fluido_selecionado, materiais_combinados, fluidos_combinados):
    ...
    # Calcula a perda de carga total na linha de sucção para a vazão de operação
...
if st.session_state.get("authentication_status"):
    ...
    # NOVO: Inicializa a lista para os trechos de sucção
    ...
    with st.sidebar:
        ...
        if col1.button("Carregar Cenário", use_container_width=True, disabled=not st.session_state.get("selected_scenario")):
            ...
            if data:
                ...
                # NOVO: Carregar dados de sucção se existirem no cenário salvo
        ...
        if st.button("Salvar", use_container_width=True):
            if project_name_input and scenario_name_input:
                scenario_data = {
                    ...
                    # NOVO: Salvar dados de sucção no cenário
        ...
        with st.expander("📚 Gerenciador da Biblioteca"):
            # ... (código do gerenciador de fluidos e materiais permanece inalterado) ...
        ...
        # NOVO: Seção na UI para os parâmetros de NPSH
        with st.expander("🔬 Análise de NPSH"):
            st.info("Parâmetros para o cálculo do NPSH disponível (NPSHa).")
            st.session_state.pressao_succao_mca = st.number_input(
                "Pressão na superfície do líquido (mca)", 
                value=st.session_state.get('pressao_succao_mca', 10.33), 
                help="Para um tanque atmosférico ao nível do mar, use 10.33 mca."
            )
            st.session_state.elevacao_succao_m = st.number_input(
                "Elevação do líquido (m)", 
                value=st.session_state.get('elevacao_succao_m', 2.0),
                help="Altura do nível do líquido no tanque de sucção em relação ao eixo da bomba. Pode ser negativo se o nível estiver abaixo."
            )
            st.session_state.pressao_vapor_mca = st.number_input(
                "Pressão de vapor do fluido (mca)",
                value=st.session_state.get('pressao_vapor_mca', 0.23),
                help="Ex: Água a 20°C tem ~0.23 mca. Este valor é crítico para evitar cavitação."
            )
            st.subheader("Curva de NPSH Requerido (NPSHr)")
            if 'curva_npshr_df' not in st.session_state:
                st.session_state.curva_npshr_df = pd.DataFrame([
                    {"Vazão (m³/h)": 0, "NPSHr (m)": 1.5}, 
                    {"Vazão (m³/h)": 50, "NPSHr (m)": 2.0}, 
                    {"Vazão (m³/h)": 100, "NPSHr (m)": 3.0}
                ])
        ...
        # NOVO: Seção na UI para configurar a linha de sucção
        with st.expander("0. Linha de Sucção (para cálculo de NPSH)"):
            for i, trecho in enumerate(st.session_state.trechos_succao):
                ...
                with st.container(border=True):
                    render_trecho_ui(trecho, f"succao_{i}", st.session_state.trechos_succao, materiais_combinados)
            c1, c2 = st.columns(2)
            c1.button("Adicionar Trecho (Sucção)", on_click=adicionar_item, args=("trechos_succao",), use_container_width=True)
            c2.button("Remover Trecho (Sucção)", on_click=remover_ultimo_item, args=("trechos_succao",), use_container_width=True)
    ...
    try:
        ...
        if vazao_op is not None and altura_op is not None:
            ...
            # ... (código para gráfico de curvas e relatório continua o mesmo) ...
            
            # NOVO: Seção para exibir os resultados da análise de NPSH
            st.divider()
            ...
            if func_curva_npshr and st.session_state.trechos_succao:
                params_npsh = {
                    'pressao_succao_mca': st.session_state.pressao_succao_mca,
                    'elevacao_succao_m': st.session_state.elevacao_succao_m,
                    'pressao_vapor_mca': st.session_state.pressao_vapor_mca
                }
                npsha, perda_succao = calcular_npsh_disponivel(
                    st.session_state.trechos_succao, vazao_op, params_npsh, 
                    st.session_state.fluido_selecionado, materiais_combinados, fluidos_combinados
                )
                npshr = func_curva_npshr(vazao_op)
                margem_seguranca = npsha - npshr

                c1, c2, c3, c4 = st.columns(4)
                c1.metric("NPSH Disponível (m)", f"{npsha:.2f}")
                c2.metric("NPSH Requerido (m)", f"{npshr:.2f}")
                c3.metric("Margem de Segurança (m)", f"{margem_seguranca:.2f}")
                c4.metric("Perda na Sucção (m)", f"{perda_succao:.2f}")

                if margem_seguranca < 0.5: # Um valor comum de segurança
                    st.error(f"**ALERTA DE CAVITAÇÃO:** A margem de segurança do NPSH é muito baixa ({margem_seguranca:.2f} m). O NPSH disponível ({npsha:.2f} m) está perigosamente próximo ou abaixo do requerido ({npshr:.2f} m). Considere aumentar o diâmetro da sucção, elevar o tanque ou verificar os dados de entrada.")
                else:
                    st.success(f"**Verificação de NPSH OK:** A margem de segurança ({margem_seguranca:.2f} m) é adequada.")
            else:
                st.warning("Adicione trechos na 'Linha de Sucção' e verifique os dados da curva de NPSH Requerido para realizar a análise.")
            ...
            # ... (código de geração de relatório continua o mesmo) ...

            st.divider()
            st.header("🗺️ Diagrama da Rede")
            # ... (código do diagrama continua o mesmo) ...
            ...
            # ... (código da análise de sensibilidade continua o mesmo) ...
